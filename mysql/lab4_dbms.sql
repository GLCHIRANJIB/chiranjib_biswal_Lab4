...
/*
Display the total number of customers based on gender who have passed orders of worth at least Rs 3000.
*/

select count(t2.cusgender) as NoOfCustomers, t2.cus_gender from
(select t1.cus_gender,t1.ord_amount, t1.cus_name from
...
9)	Create a stored procedure to display supplier id, name, rating and
 ...
 If rating >4 print “Good Service”, If rating >2 print “Average Service” 
...
(select test2.supp_id, avg(test2.rat_ratstarts) as Average from
(select supplier_pricing.supp_id, test.ORD_ID, test.RAT_RATSTARTS FROM
